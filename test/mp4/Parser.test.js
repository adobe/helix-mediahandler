  ...
  it('Parse a file that contains an MVHD atom with version 1', async () => {
    const buf = Buffer.concat([
      ...
      Buffer.from([
        ...
        0x00, 0x00, 0x00, 0x00, 0xe0, 0xdd, 0xa7, 0xa3, // creation time
        0x00, 0x00, 0x00, 0x00, 0xe0, 0xdd, 0xa7, 0xa3, // modification time
        0x00, 0x00, 0x02, 0x58, // timescale
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x35, 0x73, // duration
      ]),
      Buffer.alloc(80),
    ]);
    const info = new Parser(buf, console).parse();
    assert.strictEqual(info.duration, 23);
  });
